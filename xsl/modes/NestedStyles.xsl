...
<!--    <xsl:sequence select="map:for-each($future-separators, function($k, $v){ … })"/> didn’t work in Saxon 9.8-->
  ...
  <xsl:template match="*[@aid:pstyle]" mode="idml2xml:NestedStyles-apply" priority="2">
    <xsl:next-match>
      ...
      <xsl:with-param name="seps" as="element(idml2xml:sep)*" tunnel="yes" select="()"/>
    </xsl:next-match>
  </xsl:template>
    ...
    <xsl:for-each-group select="$nodes" group-ending-with="*[exists((self::idml2xml:sep | self::idml2xml:tab/idml2xml:sep) intersect $seps)]">
      ...
      <xsl:choose>
        <xsl:when test="exists(
          ...
          <xsl:apply-templates select="current-group()/self::Properties[../@aid:pstyle]" mode="#current"/>
          <idml2xml:genSpan>
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:apply-templates select="current-group()[empty(self::Properties[../@aid:pstyle])]" mode="#current"/>
              </xsl:otherwise>
            </xsl:choose>
          </idml2xml:genSpan>
        </xsl:when>
