...
<!--            <xsl:variable name="left-page-available" as="xs:boolean"
  ...
  <!-- Text length / word counting, especially for dropcaps etc. in nested styles -->
  ...
  <xsl:accumulator name="text-length" initial-value="()" as="xs:integer*">
    <!-- The sequence is a stack of text length. The first number is the text length so far (before 
      the current text node) of the current para, the second number, if present, is the text length
      so far of the containing para, etc. This nesting can occur when there are footnotes, 
      for example. 
    -->
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]" 
      phase="start" select="$value">
      <!-- We need to set the current value for phase="start" explicitly for text nodes. Otherwise the
        accumulator would always be the end value as long as we are inside the text node. I wonder whether
        this is a bug in Saxon’s implementation (tested with 9.8 and 10.5) -->
    </xsl:accumulator-rule>
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]"
      select="($value[1] + string-length(.), tail($value))" phase="end"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="(0, $value)" phase="start"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="tail($value)" phase="end"/>
  </xsl:accumulator>
  
  <xsl:accumulator name="text-so-far" initial-value="()" as="xs:string*">
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]" 
      phase="start" select="$value"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]"
      select="($value[1] || string(.), tail($value))" phase="end"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="('', $value)" phase="start"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="tail($value)" phase="end"/>
  </xsl:accumulator>
  
  <xsl:accumulator name="word-count" initial-value="()" as="xs:integer*">
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]">
      <xsl:variable name="word-boundary-before" as="xs:boolean" select="accumulator-before('word-boundary')[1]"/>
      <xsl:sequence select="(xs:integer($value[1] 
                                        + count(tokenize(., '[\p{Zs}\s]+')[normalize-space()])
                                        - (if ($word-boundary-before) then 0 
                                               else if (matches(., '^[\p{Zs}\s]')) then 0 
                                               else 1)),
                             tail($value))"></xsl:sequence>
    </xsl:accumulator-rule>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="(0, $value)" phase="start"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="tail($value)" phase="end"/>
  </xsl:accumulator>
  
  <xsl:accumulator name="word-boundary" initial-value="()" as="xs:boolean*">
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]" 
      phase="start" select="$value"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]//text()[idml2xml:is-para-text(., ancestor::*[@aid:pstyle][1])]"
      select="(matches(., '[\p{Zs}\s]$'), tail($value))" phase="end"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="(true(), $value)" phase="start"/>
    <xsl:accumulator-rule match="*[@aid:pstyle]" select="tail($value)" phase="end"/>
  </xsl:accumulator>
