  ...
  <xsl:template match="prop/@type" mode="idml2xml:XML-Hubformat-add-properties" as="node()*">
    ...
    <xsl:choose>
      ...
      <xsl:when test=". eq 'percentage'">
        <xsl:if test="$val castable as xs:integer and not(xs:integer($val) eq -1)">
          <idml2xml:attribute name="{../@target-name}">
...
<!--    <xsl:message select="'IIIIIIIIIIIIIII ', $prelim"/>-->
  ...
  <xsl:template match="dbk:mediaobject[(some $n in ancestor::dbk:para[not(dbk:informaltable or dbk:sidebar[@remap eq 'HiddenText'])][1]//text() satisfies (matches($n, '\S')))
                                       or
